...
| — | next: WP-63 → WP-64 (bugs) → ~~WP-60/62/61 (Part 2i)~~ done → WP-65/66/71 → WP-67/68 → WP-69 → WP-70 + community leftovers (WP-58/56/55/57) → WP-72 ADR anytime → WP-73 last; design-deck decisions for Bene: community model A/B/C (slide 11), KiCad import wizard yes/no (slide 12), plus Part 2j's three defaults to veto (rename "Parts", archive location, seed-set contents); WP-20 blocked on Inventor-machine artifacts; package + send the E2 asks (#1–11) to Ethan; the 315-record review is largely superseded by WP-68's archive-and-seed approach | ⬜ |
