...
| WP-41 | whole-module STEP binding: "whole module" toggle + "fit to cube" (bbox-centers the mesh); a new place mode with a grabbable optics gizmo writes the placed pose (position + orientation quaternion) into the record frames; the mirror size resizes the disc; a placed mirror emits front+reflected via the reflection law; multi-primitive (add/select N optics → N fragment surfaces + per-instance ports for a dual-axis galvo). Core: `TemplateRecord.provenance` + verify-t1 checks the promoted frame (E2 ask #9) | ✅ done — verified live: whole-module toggle → provenance `whole-module`, template `front` frame carries the placed rotation, ports front+reflected (45° mirror → reflected `+y`, folds 90°), fit-to-cube centers `[12,-3,7]→[-12,3,-7]`; verify-t1 green on the placed pair; 18 model + 14 core (T1/T2/T3) + 346 core tests |
| WP-42 | variable surfaces ↔ firmware: `DofSpec` gains `pivot-frame` + `surface` (E2 ask #9); `library validate` enforces the axis-map contract (E_AXIS_MAP_UNKNOWN_DOF / E_DOF_SURFACE_RANGE errors, W_ACTUATABLE_UNBOUND / W_DOF_PIVOT_UNKNOWN warnings); dual-axis galvo + motorized-z reference contracts in DOCS/LIBRARY.md (dof == fx == axis-map, one name); index carries pivot/surface/CAN per dof; editors consume it — schematic panel shows ⚡ actuated sliders (surface + pivot + CAN tooltip), `beamAxesOf` sums rotation-dof swings, the mechanics overlay tilts each placed mirror about its own pivot | ✅ done — verified live: galvo places with both dofs bound (tilt_x→0x6070 surface 0 pivot_x, tilt_y→0x6071 surface 1 pivot_y), sweeping tilt_x swings the schematic arm 90°→110° then tilt_y→120°, panel renders ⚡ tilt_x/⚡ tilt_y sliders; unknown-dof errors, unbound-actuatable warns; 351 core + 122 frontend tests |
| WP-43 | one database: `optikit-core import csv` migrates every legacy CSV row → record trios under `openuc2.*` (category from group/opticalKind, focalLength→thin-lens surfaces, GLB→template, price/docs preserved, ALL review-flagged); the palette reads the registry index only (CSV behind `localStorage['optikit-legacy-csv']`), groups by category · namespace, "Create Custom Module" routes to the component editor; DOCS/LIBRARY.md + frontend README document the one-database contract (records vs designs, how to extend) | ✅ done — verified live: palette renders 113 modules entirely from the registry (0 CSV leftovers, legacy off), category-grouped, migrated laser_405nm + mirror_1x1 place; 105 rows → 315 records, full library validates clean; 355 core + 124 frontend tests |
| WP-37 | light polish + View-3D retirement: the legacy `Editor3DPage` is deleted and `/configurator/3d` redirects to the assembly (`?from=3d` shows a "now lives here" info snackbar); the retired "Assembly (3D)" toolbar button + tooltip point at the assembly; the assembly panel links a selected insert to its optical component (`?open=<componentId>` deep-links the component editor, wired via `LibraryPaletteEntry.componentId`); working-plane/legend/ortho-flip/lock chrome moved off hardcoded slate onto `background.paper`/`divider` theme tokens | ✅ done — verified live: `/configurator/3d`→`/configurator/assembly?from=3d` with the snackbar, toolbar tooltip "The 3D cube view lives in the Assembly now (WP-37)"; tsc + 128 frontend tests |
| WP-27 | tutorials & guides: `src/components/guides/guides.ts` declares 3 cross-route walkthroughs (design a beam path · author a part record · assemble & keep in sync), each an ordered step list with per-step "go to this page" deep links; `GuidesDialog` (Help ▸ "Guides & tutorials") lists them → vertical-stepper drilldown; legacy grid-builder tour kept as a second Help item | ✅ done — verified live: Help menu opens the dialog with 3 guides, "Author a part record" drills to a 5-step stepper with navigation buttons |
| WP-26 | motor/firmware actuation runtime: `optikit-core/library/actuate.py` + `optikit-core actuate <module> <dof>=<value>` turn a DOF value into a command descriptor (CANopen SDO index/subindex view + UC2-REST `/motor_act`\|`/galvo_act` view), validating module/template/dof/actuatable/range/axis-map binding; frontend `src/model/actuation.ts` mirrors it (`firmwareCommand`, `sendActuation` POSTs to a configurable device URL) and the schematic panel gains a ⚡ send button per actuated DOF + a Device URL field (empty → shows the command only) | ✅ done — verified live: galvo `tilt_x=7.5` CLI emits the correct `/galvo_act` command, out-of-range/unknown-dof rejected; panel ⚡ send on a placed galvo prints `/galvo_act tilt_x=0 (no device — command only)`; 5 core + 4 frontend actuation tests |
| — | feedback round 6 triaged (2026-07-23, **Part 2g → `kicad-for-optics-part2g.md`**): WP-44 groups/OPM, WP-45 carriers (FRAME bay + baseplates + puzzle pieces), WP-46 fibers, WP-47 source states + SLM class, WP-48 authored SVG symbols, WP-50 live BOM, WP-51 fix bundle (composition card, index cache, theme remnants, generate button), WP-52 auto-chain E_NO_TARGET; T1/T2/T3 path verification written up there (A done, B parametric-only, C UI-thin) | 📋 |
| WP-52 | auto-chaining round 2 — diagnosis: NOT a records/traversal bug (ports correct, index surfaces them, placement derives the right grid rotation, traversal chains straight+folded when parts face right); the reported E_NO_TARGET is a genuine dead-end (detector facing the wrong way after a fold) with a useless error. Fix: core `Escape` dataclass — the walk records every dead end (no port faces the ray / hit with no exit), E_NO_TARGET carries the source's escapes + names them ("Ray leaves at: mirror_1x1.reflected → -y"), `InferResult.escapes` exposes pruned arms; service serializes escapes in the 422 + infer response; frontend maps them to markers (from-part + empty cell hint "no part at cell [1,-1,0]") and draws a dashed-red escape ray to that cell in the schematic | ✅ done (core `88cb518` + frontend `74b69de`) — the reported fold chains green when the detector faces it (pinned); the default-camera design raises E_NO_TARGET carrying the mirror.reflected escape heading -y; 363 core + 131 frontend tests. Live end-to-end needs the dev service restarted onto this build (it runs an older debugger-attached process) |
| — | feedback round 7 triaged (2026-07-23, **Part 2h → `kicad-for-optics-part2h.md`**): WP-53 miniFRAME inner-cube template + plate generator, WP-54 browse-setups → `.dsn` + share links, WP-55 GitHub docs sideload, WP-56 embedded viewer v2, WP-57 STEP assembly export (optics + beam), WP-58 community template repo + import-by-URL + asset hygiene, WP-59 OSHWLab-for-optics phased plan (A share pages / B openUC2 ordering / C social); answers: T1 STP lives next to the yaml (migrated `glb-url` split-brain → WP-58.4), WP-20 needs run logs + one golden marker-stamped .iam→STP/GLB pair from an Inventor machine | 📋 |
| WP-53 | miniFRAME inner cube + plate generator: new record kind `cube_group` (members at relative cells + rot90/overhang, envelope-grid, interface ports, `structure` block: sandwich plates + puzzle joints) with `check_groups` validation in `library validate` (envelope/overlap errors; missing-plate/undersized-plate/missing-joint warnings) and `group suggest` (minimal covering plates + joint cells from the arrangement); T3 `generators/plate_nxm.py` (4 screw holes/cell, optional apertures + corner tie-rods; harness now holds generated parts to THEIR declared envelope) with stock trios plate_3x3/3x4/3x5 generated + verified; the real Inventor puzzle piece (PRT-1004-PUZ11, 55.6×55.6×5) ingested as STEP+GLB trio; `openuc2.group.miniframe_brightfield` transcribed from the OPM-0003 screenshot (18-cube stack + 2-cube camera arm overhang) | ✅ done (core `a9b2952`) — validates clean, suggest reproduces the declared structure, removing the top plate warns, plates generate real STEP/GLB (150×150/200/250×5 mm); 371 core tests (8 new). Frontend group placement UX (place/enter/edit as one unit) remains WP-44 |
| WP-54 | setups speak `.dsn`: core `import layout-json` (legacy grid_pos/yaw → document cells + the palette's default-rotation basis mirrored in Python; unknown moduleIds flagged) verified on the real PyInventor layout; share links — `?design=<url>` (hosted) + `?d=<deflate+base64url>` (inline, native CompressionStream, 6 kB guard) loaded by the schematic once modules register, URL cleaned after consumption; "Generate Shareable Link" re-backed by the .dsn form; Browse Setups gains an "OptiKit designs (.dsn)" gallery rail (bundled index at `public/designs/`) with miniframe-brightfield (31 parts, expanded from the WP-53 group incl. plates + 9 puzzle pieces) and the converted legacy microscope | ✅ done (core `1423ac1` + frontend `<wp54f>`) — verified live: gallery cards render, "Open in configurator" places all 31 parts and cleans the URL, the loaded design re-shares as a 969-char `?d=` link that round-trips into a fresh session; 375 core + 135 frontend tests |
| WP-44/45 | groups in the editor + carriers with bays: the index exposes `groups` (members/structure/interface) and per-module `carrier`+`bays`; `addGroup` places members AND the WP-53 structure (plates + puzzle joints) as parts tagged with one instance id — the arrangement drags as ONE rigid unit (unlock for member editing, ungroup dissolves), membership rides on part params so it survives `.dsn` round trips; palette "Groups (optical modules)" section with one-click place; a drop inside a placed carrier's bay snaps the group to the bay origin with overflow warning; records: `openuc2.group.focus_lock` (IR probe → dichroic → focus camera, `to_objective` interface; new laser_850nm_ir trio), `openuc2.cube.frame` (3x3x2 miniframe bay, axis +z), baseplate_4x4/8x4 carriers; E2 ask #10 filed (groups compile to .dsn subassemblies) | ✅ done (core `bdf8837` + frontend `bbfa9eb`) — verified live: palette lists both groups, focus_lock places 4 members that drag rigidly, FRAME+miniframe drop snaps to bay origin [1,1,0] (31 parts, exact fit), panel chip/edit/ungroup work; 376 core + 142 frontend tests. Collapsed interface-port glyph rendering = follow-up |
| — | community UI (2026-07-25, from the Claude-Design review deck): screens 01–03 adapted onto real data — teal design system (`src/components/community/`), landing as the front door (`/configurator` → hero + two doors + real stats; schematic at `/configurator/schematic`), Explore gallery (filters, BUILDABLE cards, legacy door), per-design page (Star/Fork & edit/Download + live BOM joined to registry prices; unpriced lines stay honest); core exposes `ModuleRecord.price` in the index (95 priced); StartupDialog retired from community routes. Money copy stays quiet pending the deck's A/B/C decision (slide 11); KiCad/OSHWLab import chips marked planned (slide 12 Q3) | ✅ done (frontend `97c7e57` + core `4c47f27`) — verified live: hero + stats render, explore filters, miniframe-brightfield BOM lists 31 parts, Fork & edit loads them into the editor; 144 frontend + 376 core tests |
| WP-46 | fibers: `PortSpec.coupling` + a design-level `fibers:` block (from/to, core-um, na, length-m, SM\|MM); chain inference follows a cord from either connector, and the compiler advances by the FIBER's length instead of the connector gap — skipping the facing/kink/transverse checks that only mean anything in free space; checks E_FIBER_ENDPOINT/LENGTH + W_FIBER_FREE_SPACE_PORT/W_FIBER_NO_NA; frontend `fibersStore` round-trips through both export paths, the schematic draws a sagging amber catmull-rom (deliberately unlike a beam), a fiber tool lays a cord pin-to-pin, and the panel edits core/NA/length/type; `laser_fcsm_1x1` declares `coupling: fiber` on its FC/APC output | ✅ done (core `4c47f27`+`ba5b7cd`, frontend `1d27917`) — verified live: a laser 400 mm off the beam line chains to the camera THROUGH the cord; the editor's exported design re-enters core unchanged (50 µm / NA 0.22 / 2 m → 12.7° divergence) |
| WP-47 | source states + programmable surfaces: `SourceSpec` line list on records, `CompSpec.enabled`/`wavelength-um` as placement state — inference SKIPS an off source so the netlist matches the bench; new `slm`/`display` categories with `ProgrammableSpec` (mode, pitch, resolution, fill factor), traced as a plane (the pattern is not simulated) with their own glyph colour; panel gets an on/off switch, a wavelength picker and a pixel-facts block; `openuc2.slm.dmd_45` trio (45° fold, `pattern_index` over CAN); the migrated `dmd_slm_4500` was classed as a **source** because the CSV had no SLM class — re-classified with review notes rather than invented datasheet numbers | ✅ done — round-trip verified: source off → empty chain + "source is off — skipped"; the DMD panel reads "reflective · 1920 × 1080 px · 5.4 µm pitch · 10.4 × 5.8 mm active". Pixel facts are enforced on the RECORD (`library validate`), never per placement — a design references the record and must not duplicate it |
| WP-48 | authored schematic symbols: `ComponentRecord.symbol` ships an SVG next to the record, served through the index; derived glyphs stay the default and a symbol is LOOKS ONLY (pins keep coming from `optics.ports`); `library/symbols.py` enforces the contract (E_SYMBOL_MISSING/NOT_SVG, W_SYMBOL_NO_VIEWBOX, W_SYMBOL_FIXED_COLOR — `currentColor` so it themes both ways); the frontend fetches + **sanitizes** it (scripts, inline handlers, `javascript:` URLs, foreignObject stripped before innerHTML) and falls back to the derived glyph when the asset is unreachable, so an offline registry never blanks a part; the ESP32 board is the proving set | ✅ done — sanitizer exercised in the real browser against every payload class; 407 core + 151 frontend tests |
| WP-50 | live BOM: `src/document/bom.ts` groups placed parts by library id and keeps their GRID CELLS, joined to registry prices, T-class, namespace and the record's review flag (unpriced lines stay visibly unpriced and contribute nothing — prices are never invented); ONE generator, two consumers — the WP-18 release bundle writes `BOM.csv` from the same function the panel renders; `BomDialog` mounts from BOTH the schematic bottom toolbar and the assembly panel (sortable, per-row `?open=` deep link, a cell chip per instance that cross-probes into the scene, CSV download); the legacy appStore `BOMPanel` is unmounted and deleted | ✅ done (frontend `2d0f68e`) — verified live: 2× laser + 1 mirror → laser qty 2 at cells [0,0,0] and [0,3,0], €650 each + €50 = €1350; clicking the [0,3,0] chip closes the dialog and the panel then reads "grid cell [0, 3, 0]" (the second instance, not the first); 155 frontend tests |
| WP-51 | fix bundle: module composition card in the assembly panel (component@ver + template@ver + STEP/GLB + electronics axis-map, each deep-linking its editor — "cube + insert + part" in ONE place) plus a collapsed two-line form in the schematic panel; T-class chips on placed parts, sourced from the INDEX rather than the palette entry (the assembly never mounts `PartLibrary`, so an entry-derived chip silently never rendered there — caught live); mtime-keyed `/v1/library/index` cache with a `?fresh=1` escape hatch; theme sweep of the ray/fiber toggles, the chaining/fiber hint chips and the spot-diagram plot | ✅ done (core `c062325` + frontend `2d0f68e`) — index cold 673 ms → **warm p50 11.9 ms** (pinned by a service test); zero hardcoded chrome colours grep-able in schematic/assembly/bind; the T3 generate button is deliberately left to WP-61 per the Part 2g note |
| — | **Part 2g fully landed** (WP-44…48, 50…53) | ✅ |
| WP-58 | community template repo + import-by-URL + asset hygiene: **new repo** at `../optikit-community-template` (record trios + a 2-cube worked design + docs + README/CONTRIBUTING as teaching material + CI that validates, refuses remote-only meshes, and **commits** the `library-index.json` a fork gets mounted from). **"Add library from GitHub"** mounts a fork: its parts join the palette badged with the repo, its designs join the gallery, and the WP-54 share link already covers them. Precedence is builtin < mounted < local drafts — a community repo can ADD parts but **never override a curated `openuc2.*` id**; a clash keeps the curated record and is REPORTED in the palette. Core: `library vendor-assets [--dry-run]` + the fix for the bug behind the split brain (`_module_assets` emitted the GLB URL unguarded, so the index promised 64 assets that 404) + the `uc2_puzzle` record whose glb/glb-url named an Inventor `.ipt` SOURCE file | ✅ done (core `7988500` + frontend `d9f277f` + template repo `8a269d4`) — verified: template repo validates clean and a broken design exits 1; merge/precedence exercised live (community part badged, its claim on `openuc2.cube.mirror_1x1` rejected and reported); both error paths give actionable messages; one asset vendored live (292 KiB glTF, index now serves it). **Open: publish the template repo to GitHub** (the mount happy-path needs a real remote), and the bulk vendor of the other 62 assets — ~18 MB into git, so the size/LFS call is Bene's |
| WP-57 | STEP assembly export: `optikit-core export step` + `POST /v1/export/step` (streams `model/step`) + two File-menu entries. Three kinds of geometry, each under a node named after its part — the placed module's template STEP, optics **lathed from the prescription** (a lens is the volume its own radii/conic/thickness/semi-aperture describe; a mirror a disc; a detector a sensor plate), and `beam_path` as swept cylinders (`--beam wires`/`off`). WP-18 determinism: sorted emission + pre-OCCT rounding make two **separate invocations** byte-identical apart from the header timestamp `rebuild` skips — with the honest caveat, documented in the module, that two exports *inside one process* differ by OCCT's global occurrence counter (geometry identical, only ids move) | ✅ done (core `81177df` + frontend `0820bc6`) — verified on fluo-scope: 4 lathed optics + 11 beam segments across both paths, products named `tube-lens/optic`, `excitation/seg-01`…, units MILLI; edge sag pinned against the conic formula computed independently; 423 core tests (10 new) |
| — | feedback round 8 triaged (2026-07-26, **Part 2i → `kicad-for-optics-part2i.md`**): the free-placement question answered — the component/template/module split already allows placing an unbound optical primitive, and the engine already tolerates template-less parts (`drc()` skips them, the residual is unclamped); measured state 117 T1 / 2 T2 / 3 T3, with 4 published symbols no module binds (incl. `thorlabs.lens.ac254-050-a`). Three wiring links: **WP-60** unbound symbols in the palette (`index.components` is fetched and never used), **WP-61** `/v1/generate` + a "generate a holder" action that materializes template+module at the placed pose, **WP-62** `solid_from_prescription` — an optiland surface stack becomes a real solid whose primary job is to be SUBTRACTED as the holder cavity (`part_step` becomes optional in `boolean_holder_1x1`), giving `library/pocket.py` its first consumer | 📋 |
| WP-60 | unbound symbols in the palette: the index's component entries now ship ports (local mm), source lines, the authored symbol AND the record's own fragment surfaces (`dump_index_json` keeps `.inf` radii strict-JSON, 1e999); `entriesFromComponents` offers every symbol NO module binds — grouped "<category> · unbound", badged UNBOUND, a module-bound optic never offered twice; placement rides the proven template-less path (continuous mm, no cell claim, DRC-invisible — pinned in BOTH repos incl. cell-sharing with a cube) and the export carries the REAL prescription, not the thin-lens guess; assembly draws a distinct translucent UNBOUND ghost (≠ the missing-mesh "no template" ghost) and the panel shows "no mechanics bound" + vendor/EFL + the WP-61 verb | ✅ done (core `4295c39` + frontend `cd9cb85`) — verified live: `thorlabs.lens.ac254-050-a` placed from the palette, chained laser → lens → camera, simulated **f2 = 50.12 mm** (the real doublet, 61 traced polylines), check/DRC clean, cubify shows the residual unclamped and no cell claimed; 445 core + 173 frontend tests |
| WP-62 | prescription → boolean solid: `generators/optic_solid.py` revolves the surface stack (exact arcs for spherical surfaces — a sampled-polyline revolve made OCC's boolean silently return EMPTY halves; splines broke MakeOffsetShape) reusing the pocket sag math, extracted to stdlib-only `library/sag.py` so the `uv --no-project` interpreter can file-path-load it; `boolean_holder_1x1` takes `part_prescription` (component id, harness-inlined before keying, or inline surfaces) as the alternative cut body, exactly-one via top-level `oneOf` (now understood by `validate_params`); meta records `derived_from` + surface digest + review sentence; sub-0.2 mm or negative edge = `E_EDGE_THICKNESS` naming the surfaces. Bonus: real runner bug fixed (bbox measured AFTER STL export read a 25.4 mm lens as 33.4 mm) | ✅ done (core `9149729`) — cavity sag agrees with `sag_at()` within **9 µm** at the clear semi-diameter (acceptance: 20), centre/edge match `pocket_params` exactly (11.5 / 8.709 mm), the holder-from-prescription passes the 50 mm envelope and clears/hugs the glass (∩ ≈ 0; +0.35 mm grown glass collides), the achromat trips the guard at **−1.461 mm**, and the same surfaces export as a standalone lens STEP; 15 new tests |
| WP-61 | "put it in a cube": `POST /v1/generate` (template_id \| component_id, params, files+component → `pose_params_from_component` folds the PLACED δ/ΔR in; sha256 cache honoured; 501 `E_NO_CADQUERY`; base64 artifacts incl. the new UNION `model.{step,glb}` for two-half runs) + the editor's "generate a holder…" on an unbound part: `GenerateHolderDialog` previews the halves/bbox/envelope verdict, accept MATERIALIZES `user.tpl.holder_*` (generator + the run's canonical params → regenerate is a cache hit; prescription runs review-flagged) + `user.cube.*_t3` through the existing exits (zip / dev-write + `bumpLibraryIndex`), then re-points `libraryRef` (`repointPartLibraryRef`) as ONE undo step — undo restores the unbound placement, artifacts stay keyed on disk. Supersedes the WP-51 "T3 generate button" line item | ✅ done (core `3b407e3` + frontend `cd9cb85`) — verified live end-to-end: generate on the placed ac254 → key `aa43f2006cb5`, 49.4×49.4×30 mm, fits-envelope; accept wrote 3 files, `library validate` OK, the panel flipped UNBOUND → **T3** with the composition card resolving both new records; identical params re-POST is a cache hit (pinned); note: the dialog's 3D half-preview hit WebGL context loss in the embedded test browser (env limit), data verified well-formed glTF |
| — | feedback round 9 triaged (2026-07-26, **Part 2j → `kicad-for-optics-part2j.md`**): answers — the STEP→function/origin/direction association EXISTS (bind workbench; the gap is housing-without-a-cube → WP-67), puzzle joints are ordinary parts placed by `addGroup` in the 5 mm interface zone, T1-only is the already-specced Part 2i wiring, "Assembly (3D)" is a WP-37 shim to remove. Diagnosed the simulate 500: our invented `{type: "ideal"}` material tag vs optiland's class-name registry, baked into 315 records by `csv_palette.py:122` + emitted by `convert.ts:83` → **WP-63** (fix first). New: **WP-64** fit & finish (pane-resize bug, duplicate tab, part→library deep link, interface-part glyphs), **WP-65** KiCad-style layer visibility, **WP-66** schematic module list + swap dropdown (amends WP-50), **WP-67** "Parts" editor rename + housing-only binding (footprint_grid optional), **WP-68** starter library (archive the 315, seed ~6 exemplary parts), **WP-69** legacy retirement (grid builder + Konva + CSV frontend, lint baseline → 0), **WP-70** publish→Explore loop (Store-repo CI index + live gallery fetch + prefilled-PR publish), **WP-71** ad-hoc grouping (shift-select, Ctrl+G, name), **WP-72** accounts/user-repo storage ADR (GitHub App vs OAuth vs Google), **WP-73** UX overhaul incl. the OSHWLab website↔editor split (deliberately LAST) | 📋 |
| WP-63 | materials speak optiland: the "Unknown material type: ideal" 500 was our own invented tag (csv_palette + paletteOpticsOf) vs optiland's class-name registry. Four layers: `compile_path` normalizes every emitted material ({type:ideal,name}→Material, {type:ideal,index}→IdealMaterial, registry dicts untouched, anything else → typed `E_MATERIAL` naming component+surface); both emitters now write the registry form; `library migrate-materials` rewrote the stock in one ruamel pass (8 lens records carried it; review flags untouched); `/v1/simulate`+`/v1/optimize` wrap `Optic.from_dict` → 422 `E_OPTIC_INVALID` naming the offending surfaces, and optimize validates the BASELINE loudly (its merit() swallows per-trial failures — a broken material would have been a silent all-1e9 "optimization") | ✅ done (core `f6208e8` + frontend `ff96a4d`) — a migrated library lens compiles and passes `Optic.from_dict` (pinned); bare-ideal raises E_MATERIAL naming `lens [0]`; hand-broken material → 422 with the surface named; 451 core + 173 frontend tests (6 new) |
| WP-64 | fit & finish: the collapse-to-white bug was NOT R3F — the persistent MUI Drawer ROOT kept its width when closed (only the paper hides), reserving a 380 px dead strip; drawer roots now release their width (+ `minWidth: 0` on canvas boxes) and R3F's own ResizeObserver does the rest. "Assembly (3D)" shim removed (one Assembly entry; /configurator/3d redirect stays). Schematic property panel gains the WP-37 `?open=` deep link into the parts editor. Interface parts stop being a circle cloud: `interfaceKindOf()` (plate/puzzle/baseplate, wellplate-safe patterns + carrier fallback) drives distinct flat glyphs with dim labels in both scenes | ✅ done (frontend `bb00e26`) — verified in-browser: both drawers, both directions, two window sizes, canvas measured 0→1280 full-bleed; deep link lands on the record; plate/puzzle/baseplate visibly distinct; 178 tests (5 new). Noted pre-existing: `/v1/library/assets/.../component.yml` 404s on the running stale service, so `?open=` shows a blank draft until it's restarted |
| WP-68 | starter library: **374 records → 101 live + archive** (after restores 107/288-ish live/archived; the keep-set computed, not hand-picked: non-migrated + named exemplars + fixpoint closure of group/kept references — 27 migrated records survive via the miniframe/focus-lock groups). Six hand-authored seeds at the quality bar (zero flags, teaching descriptions): `starter_50mm` (radii derived so optiland traces EFL **50.000** exactly — the WP's thin-lens ±51.68 gives 50.60 at 3.6 mm; the record teaches both formulas), `z_motor` (T2 + WP-42 axis-map exemplar), `laser_basic` (WP-47 lines), `led_basic`, `camera_basic`, kept `mirror_45`. `library/archive/**` skipped by the loader; `library restore <id>` brings a trio back (module drags component+template); a ref that names an archived record fails `E_ARCHIVED` naming that exact command. `sampleholder_1x1` + `laser_488nm` restored (bundled demo + the user's open design use them). Frontend: snapshot rebuilt (33 modules/37 components vs 117/126), legacy-microscope demo remapped to seeds | ✅ done (core `2386bb0`+`5076d09` + frontend `1326174`) — `library validate` fully finding-free (the dmd_slm warning left with the archive); restore round-trip pinned; 457 core + 178 frontend tests |
| WP-65 | layers, KiCad-style: `layerOf(part)` (= cell z) + interface classification (plates/joints/baseplates belong to the interface ABOVE their layer, clamped into the design's span) in src/document; a persisted store (per-layer visible/dimmed, solo, "plates & joints" sub-toggle) with ONE precedence rule both editors share: sub-toggle → **active working-plane layer always visible** (the KiCad placement guard — the floor you're editing can't be hidden) → solo → per-layer override. Hidden parts unmount (no raycast, deselect-on-hide so Del can't nuke invisibly); dimmed parts render faint and non-interactive; beam paths break into runs between visible ports (nothing bridges a hidden part), fibers with a hidden end vanish, the 2D ray preview deliberately untouched (physics, not view). Compact chip row in both editors (click cycles visible→dimmed→hidden) | ✅ done (frontend `b611b1e`) — verified live on the 31-part two-layer miniframe: hide L1 / solo L1 / sub-toggle / drop-while-hidden lands visibly on the active plane / persistence across reload in BOTH editors; 188 tests (10 new) |
| WP-66 | schematic Modules panel (amends WP-50): Design \| Modules tabs in the right drawer; one row per placed part (ref · module · cell · layer · T-class · group chip) with two-way cross-probing and an aggregate toggle rendering the SAME `buildDocBom` generator (two projections, numbers can never fork). `swapPartModule()` is the swap semantic in one function: `repointPartLibraryRef` (pose/ref/params untouched, defaultRotationFor deliberately not re-applied), whole chains dropped only where the new record lacks the port name (warning toast), DOF values re-clamped or REMOVED (new `clearDofValue` — the removal path `setDofValue` never had), all ONE undo step; same-category-first dropdown with "all modules" expander; locked group members refuse per the WP-44 rule | ✅ done (frontend `edbf9e5`) — verified live: swap mirror_1x1→mirror_45 kept pos/cell/rot24 bit-identical and bumped the revision; undo restored; aggregate shows 2× laser qty 2 · €1300; 196 tests (8 new) |
| WP-69 | legacy retirement: **47 files / 10,627 LOC deleted** in one reviewed sweep — the Konva 2D builder (EditorPage/Layout/GridCanvas/panels/overlays), the module wizard, the intro.js tour, the dead WP-37 `three/` view (+ simulationStore + worker; `SimulationEngine` stays — the schematic preview and RaySketch use it), the CSV plumbing (`csvHandler` **embedded a GitHub PAT** — gone), ChatPanel with the dock that mounted it. appStore halves (1955→983 lines) via a zero-importer sweep; `loadModules` is registry-prune only; SetupBrowser verified independent (its CSV is setup metadata) and stays. `/configurator/grid` → schematic redirect with a one-time notice; Toolbar drops Grid/Annotate/grid-only items; PartLibrary drops Create Custom Module. konva/react-konva/intro.js uninstalled | ✅ done (frontend `7d5c273`) — main chunk 2131→1548 kB (−27%, gzip 620→448); **`npm run lint`: 46 → ZERO findings** with no blanket disables (remaining kept-file hits fixed properly) — CI can enforce it now; 191 tests; schematic/assembly/parts/setups/redirects smoke-tested live |
| WP-74 | spectral & polarization response (the physics gate): a `response:` block on a fragment surface (`kind` ∈ mirror/sampler/polarizing/dichroic/absorptive, `reflect-bands-um`, `transmit-bands-um`, `transmit-ratio`, `polarization-deg`), validated explicitly but stored verbatim in the Optiland fragment. `chain/infer.py` threads the source line through the walk and prunes an arm the wavelength cannot take (the reflect/transmit split read from the port-role the record names, since a 45° fold makes both exits perpendicular to the incoming beam); `checks.py` gains `W_FILTER_STACK`; new `budget.py` computes the **photon budget** (fraction/mW to the sensor — the BOM analogue), surfaced in `/v1/simulate` + the service panel. Jones declared, not projected. Seeds: real DMLP505 dichroic + `emission_525` band-pass filter | ✅ done (core `cc86aa2` + frontend `f590272`) — the golden fluo-scope now infers exactly its two authored paths (the dichroic bleed-through is pruned by band at 488, flips at 525) and reports 20 mW to the sensor; `library validate` finding-free; 469 core + 191 frontend tests (test_response.py new) |
| WP-79 | the lossy channel — authored data must reach the engine: three fields were authored then dropped. Frame **rotation** (a bound 45° mirror simulated axis-aligned — the index's `_component_ports` never read the quaternion) and **clear aperture** now ship through the index and are preserved by `paletteOpticsOf` (a named frame is created for a rotation/aperture, not only an offset); the **interaction model** is no longer hardcoded to `RefractiveReflectiveModel` — `_normalize_interaction` carries an authored `ThinLensInteractionModel`/coating through, normalized onto optiland's registry, with typed `E_INTERACTION` (the WP-63 pattern, and the WP-75 groundwork); and `DRC_APERTURE` — a stub returning `[]` since the beginning — gets its first check: the intra-cube decenter transverse to a port axis must stay inside the aperture radius | ✅ done (core `4543dc1` + frontend `e3c2db0`) — a bound 45° mirror placed from the palette folds the beam in simulation (pinned); a gross decenter raises DRC_APERTURE naming the port; 475 core + 192 frontend tests (test_lossy_channel.py new) |
| WP-80 | rotation DOFs reach geometry: a rotation DOF was declarable, validated, firmware-bindable, sliderable, sendable, and back-annotatable — then the compiler dropped it ("only translation DOFs move poses yet"). `_apply_dof_values` gains a rotation branch (tilts the component in its local frame, ports/frames swing rigidly, downstream geometry follows); `cubify._check_t2_offsets` projects the residual tilt onto declared rotation DOFs; `fx.py` emits `value-deg`; `listPartMechanics` exposes `rotationDofs` and the assembly panel lists them. **Documented limitation:** a rigid θ rotation — a fold mirror's 2θ reflection-law doubling and the per-surface dual-axis galvo case need the compiler to compute reflection from surface normals (a follow-on) | ✅ done (core `6265e9b` + frontend `5e5aba7`) — the mirror's compiled surface pose changes with the DOF instead of staying axis-aligned (pinned); back-annotation's TILT_UPDATE now round-trips through a re-compile; 479 core + 202 frontend tests |
| — | feedback round 12 triaged (2026-07-29, **Part 2m → `kicad-for-optics-part2m.md`**): the spoken vendor-workflow walkthrough + the three benchmark instruments. **Coverage check: most of it is already built** — vendor zmx→cube (`import zmx` + WP-82 UI + WP-61), STEP assembly + reflective-datum assignment (Parts mechanics tab, WP-83's KM05 ref), laser-housing association (WP-67), primitives onto existing Inventor cubes + export optimized poses back (WP-84/85), prescription→volume (WP-62/75). **Four genuinely new**: **WP-93** the optimizer becomes a design tool — free-space POSE variables for unbound optics (`_free_dofs` varies only declared DOFs today, so "break it loose so the optimizer moves it" silently does nothing), per-variable constraints (bound/fix/link), a merit CHOICE (the merit is hardcoded to RMS spot at `app.py:557-570`), and a "change the holder…" verb (T1↔T2↔T3 in one undo step) so optimizer-moved parts get new mechanics; **WP-94** virtual detectors — non-physical probes anywhere in the beam (incl. inside a sample volume) returning an irradiance map + **homogeneity**/power/D86, no cube, no BOM, never perturbing the trace, feeding WP-93's merit; **WP-95** the benchmark suite — fluo-scope (exists, gains optimization+probe acceptance), **light sheet** (two independent arms at the "aquarium"), **confocal** (galvo 2θ + pinhole gating) as goldens + CI; plus a **WP-90 amendment** for multi-axis DOF authoring + a generic `openuc2.tpl.lens_holder_xyz` (every T2 template today declares exactly ONE translation axis). Scheduled as **M9 · MVP PROVEN** — the gate where three real instruments go green end to end. E2 ask: `probes:` as a design-level block | 📋 |
| — | feedback round 11 triaged (2026-07-29, **Part 2l → `kicad-for-optics-part2l.md`**): answers — ports STAY (they are the KiCad pins: chain inference, compilation and the WP-74 spectral gate all walk them; the gap is legibility, → WP-89); inference's 20° facing tolerance is deliberately tight (after grid placement a real target is ~0° off, a non-target ≥90°; 45° would match perpendicular ports); sources are already multi-line (`SourceSpec.wavelengths_um`); attach Inventor mechanics to an optics-only record via WP-67 housing / WP-84 attach. New: **WP-87** import an Optiland setup JSON → a row of unbound free primitives (no T-class until cubify, each a promotable temp component — the third import road beside zmx/glb), **WP-88** sequential beam-path scripting (Optiland/PyOpticL-style DSL, two-way with the canvas), **WP-89** the part inspector shows the optics (EFL/aperture/surfaces/lines + a legible port list — makes ports visible), **WP-90** Parts editor per-category authoring (a mirror gets a substrate not a lens's radii; rectangular apertures; a `mechanics:` ref; template-class routing), **WP-91** palette list view + multispectral source representation, **WP-92** editor bug sweep round 2 (WebGL context loss after the holder preview kills the canvas + hides the generated cube; decimal-comma inputs → NaN; unbound-lens yaw snaps to N·90° with snap off; puzzle/plate glyphs still invisible). Slotted into Phases 1–4, all before the community/UX phase. E2 ask: WP-90's `mechanics:` component→housing ref | 📋 |
| WP-82a | **reflection from normals** (carved out of WP-80 while implementing it, 2026-07-29 — the `a` marks an amendment to WP-80, NOT a sub-package of WP-82): WP-80 ships a rigid θ rotation, right for a refractive tilt / tip-tilt platform / rigid kinematic mount but **wrong by 2× for a reflective surface** (tilt a mirror θ → the beam must deviate 2θ; the exit port rotates rigidly with the component so it leaves θ short), and it leaves `DofSpec.surface` inert so a dual-axis galvo's mirrors cannot steer independently. The reflection law already exists as a WP-40 **validator** (`library/derive.py:83-84`); this promotes it to the geometry source whenever a reflective element is off nominal, adds per-surface tilts for DOF-named surfaces, and self-checks by asserting derived == authored at the nominal pose (`E_PORT_NORMAL` otherwise) so correctly-authored designs compile unchanged. Sharpest motivation: **the browser's 2D preview already applies 2θ** (`ports.ts::beamAxesOf`), so the fast approximation and the authoritative trace disagree today — and the approximation is the correct one. Curved reflective surfaces stay out of scope (`E_UNSUPPORTED`). Full prompt: Part 2k amendment section | 📋 |
| — | **MVP roadmap consolidated** (2026-07-28, **`kicad-for-optics-roadmap.md`**): all open packages from Part 2j (WP-67/70/71/55/56/59/72/73) + Part 2k (WP-74…81) + five new (WP-82 CLI→UI import/viewers, WP-83 reference-parts corpus, WP-84 Inventor identity-preserving re-import, WP-85 PyInventor T2 bridge + human test, WP-86 hardware calibration write-back) reordered into ONE dependency-driven, MVP-first sequence organized around driving Scenarios 1–5 fully through the UI. Phases: (1) physics correct & lossless [WP-74/79/80] → (2) the verbs in the UI [WP-76/77/81/75] → (3) part-first + test bed + round-trip [WP-67/83/84] → (4) CLI→UI + ergonomics [WP-82/78] → (5) the human-tested PyInventor T2 loop [WP-85] → (6) grouping + hardware closure [WP-71/86] → (7) community/accounts/UX LAST [WP-70/55/56/59/72/73]. Includes the integration flowchart (five roads → one symbol → free placement → T1/T2/T3 cubify → produce → measure-back) and a Go-divergence note filed in GO_INTEGRATION.md. **Correction to earlier passes:** the PyInventor scripts (`apply_fx_params.py`, `batch_iam_to_stp_glb.py`) + `MAS-2000-CUSTOM.stp` DO exist at `~/Downloads/PyInventor` — the T2 Inventor half is real, just never tested against a live Inventor (that is WP-85's human deliverable, also unblocking WP-20) | 📋 |
| — | feedback round 10 triaged (2026-07-28, **Part 2k → `kicad-for-optics-part2k.md`**): answers — an achromat/objective IS a fragment ("optics group" = the surface stack) and compiles today incl. air gaps, so the real gaps are solid generation for air-spaced groups, the compiler's hardcoded `interaction_model` (a paraxial black-box objective is silently discarded), and the barrel housing (= the queued WP-67); "put an unbound optic in a cube" shipped as WP-61 but only in the Assembly view; manual chaining is already optional (`inferAndAdoptChains` exists) but modal and text-only; the component editor's T3 dropdown emits a **generative template with no `generator:` block** — a dead record. New: **WP-74** spectral/polarization `response:` block (inference pruning + filter-stack check + photon budget — the PyOpticL finding, confirmed independently: after WP-68 the live library has NO filter category at all), **WP-75** optics groups (air-spaced solids per glass group, paraxial black boxes, element-aware editor), **WP-76** the unbind verb (cube → free primitive, the inverse of WP-61; builds on WP-66's `swapPartModule`), **WP-77** component-editor T3 + the dead-record fix + live EFL, **WP-78** context menu / copy-paste / chaining that proposes, **WP-79** the lossy channel (frame rotation, clear aperture and interaction model are authored then dropped — a bound 45° mirror simulates axis-aligned today; implements DRC_APERTURE), **WP-80** rotation DOFs reach geometry (back-annotation can WRITE a rotation dof_value the compiler then ignores), **WP-81** the optical-axis boolean (holders currently seal their own beam path) | 📋 |
| — | next: ~~WP-63 → WP-64 (bugs)~~ done → ~~WP-60/62/61 (Part 2i)~~ done → ~~WP-65/66~~ done / WP-71 → WP-67 (Parts editor + housings; the rename decision is confirmed) → ~~WP-68~~ done → ~~WP-69~~ done → WP-70 + community leftovers (WP-58/56/55/57) → WP-72 ADR anytime → WP-73 last; design-deck decisions for Bene: community model A/B/C (slide 11), KiCad import wizard yes/no (slide 12) — the Part 2j defaults (rename "Parts", same-repo archive, seed set, CSV deletion) are **confirmed**; WP-20 blocked on Inventor-machine artifacts; package + send the E2 asks (#1–11) to Ethan; the 315-record review is superseded by WP-68's archive-and-seed (done) | ⬜ |
