...
| WP-58 | community template repo + asset hygiene (partial): **new repo scaffolded** at `../optikit-community-template` — record trios + a 2-cube worked design + WP-55-consumable docs + README/CONTRIBUTING as teaching material + CI (`library validate`, per-design validation, a refusal of remote-only meshes, and it publishes the index a fork gets mounted from). Core: `library vendor-assets [--dry-run]` pulls remotely-referenced meshes next to their record, and **the bug behind the split brain is fixed** — `_module_assets` emitted the GLB URL unguarded, so the index promised 64 assets that 404 (now `is_file()`-guarded like step/symbol). Data fix: `openuc2.tpl.uc2_puzzle` had glb+glb-url set to an Inventor `.ipt` SOURCE file — dropped, kept as provenance, flagged as a likely duplicate of WP-53's `puzzle_1x1` | 🔶 partial (core `7988500` + template repo `6f217de`) — verified: template repo validates clean and a broken design exits 1; one asset vendored live (292 KiB glTF, index now serves it). **Remaining: import-by-URL in the configurator (item 2), gallery from mounted repos (item 3), and the bulk vendor of the other 62 assets — ~18 MB into git, so the size/LFS call is Bene's** |
