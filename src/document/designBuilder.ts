...
export interface BuildDesignOptions {
  ...
  name?: string;
  /** Library index hash stamped as `provenance.catalog` (decision 6.11): an
   * importer compares it against the live index to tell whether the records
   * this design inlined have drifted since export. */
  catalogLock?: string;
}
...
export function buildDesign(
  ...
  const design = {
    ...
    components,
    ...(options.catalogLock
      ? { provenance: { catalog: { 'index-hash': options.catalogLock } } }
      : {}),
  };
