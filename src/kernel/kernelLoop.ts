...
import type { UnmappedPlacement, WorldPose } from './pose';
import { poseDelta, samePose } from './pose';
  ...
  private async scene3WithRetry(yaml: string, id: number): Promise<Scene3Response | null> {
    ...
    for (let attempt = 0; ; attempt++) {
      try {
        ...
        // status 0 = the client's E_UNREACHABLE wrapper around a network
        // failure — retriable like any transport error; 4xx (incl. 422) never.
        const retriable =
          !(error instanceof CoreServiceError) || error.status >= 500 || error.status === 0;
