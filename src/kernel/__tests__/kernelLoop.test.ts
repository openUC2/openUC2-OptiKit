...
describe('KernelLoop', () => {
  ...
  it('reports a 422 without retrying (same input, same answer)', async () => {
    const { loop, results, errors } = harness({
      scene3: () => Promise.reject(new CoreServiceError('E_NO_TARGET', 'no chain', '', 422)),
    });
    await loop.flush();
    expect(loop.requestsIssued).toBe(1);
...
/** A grid-cell pose in the document frame (50 mm pitch), upright. */
const poseAt = (x: number, y: number): WorldPose => ({
  positionMm: [x * 50, y * 50, 0],
  rotation: [0, 0, 0, 1],
});
